...
function App() {
  ...
  const handleCheckout = async () => {
    ...
    if (failedReservations.length > 0) {
      alert(`Some items are no longer available: ${failedReservations.map(r => r.error).join(', ')}`);
      loadProducts();
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-zinc-900 bg-[radial-gradient(circle_at_20%_10%,rgba(251,191,36,0.14),transparent_45%),radial-gradient(circle_at_80%_15%,rgba(0,0,0,0.06),transparent_35%)]">
          ...
          <div className="absolute inset-x-0 bottom-0 p-4 sm:p-6">
            <div className="max-w-2xl">
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button className="btn bg-white/20 text-white border-white/25 hover:bg-white/25">
      ...
      <footer className="text-center py-8 mt-16 container-x space-y-4">
        <div className="flex items-center justify-center gap-2 mb-2">
          <span className="muted text-xs">Biztonságos fizetés:</span>
          <img
            src={siteSettings?.footer_logo || '/medium-nobg-light.png'}
            alt="Barion Payment"
            className="h-10 object-contain opacity-70 hover:opacity-100 transition-opacity"
          />
        </div>
        <p className="muted text-xs tracking-widest uppercase">
          Vintage Vibes — Alapítva 1999
        </p>
      </footer>
    </div>
  );
